...
export default function Only1FrontendPageDespite29BackendRoutesSevPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-only-1-frontend-page-despite-29-backend-routes-sev/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Only 1 frontend page despite 29 backend routes — severe UI gap</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-only-1-frontend-page-despite-29-backend-routes-sev/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
