...
export default function DespiteAiJsAistreamJsRouteFiles0MountedAiPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/gap-despite-ai-js-aistream-js-route-files-0-mounted-ai/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Despite ai.js / aiStream.js route files, 0 mounted AI endpoints — AI layer missing</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-despite-ai-js-aistream-js-route-files-0-mounted-ai/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
