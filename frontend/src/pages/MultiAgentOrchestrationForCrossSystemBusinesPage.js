...
// Feature: Multi-agent orchestration for cross-system business processes with audit trail
import React, { useState } from 'react';
import api from '../services/api';

export default function MultiAgentOrchestrationForCrossSystemBusinesPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-multi-agent-orchestration-for-cross-system-busines/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Multi-agent orchestration for cross-system business processes with audit trail</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-multi-agent-orchestration-for-cross-system-busines/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
