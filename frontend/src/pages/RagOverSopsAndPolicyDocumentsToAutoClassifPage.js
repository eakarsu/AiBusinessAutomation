...
// Feature: RAG over SOPs and policy documents to auto-classify and route incoming work
import React, { useState } from 'react';
import api from '../services/api';

export default function RagOverSopsAndPolicyDocumentsToAutoClassifPage() {
  ...
  const run = async () => {
    ...
    try {
      const r = await api.post('/cf-rag-over-sops-and-policy-documents-to-auto-classif/run', { input });
      setResult(r.data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>RAG over SOPs and policy documents to auto-classify and route incoming work</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-rag-over-sops-and-policy-documents-to-auto-classif/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
      <textarea
        value={input}
